...
function plot_heteroclinic_acceleration(path, dt)
    path = path[1:3, :]
    velocities = diff(path, dims=2) ./ dt
    accelerations = diff(velocities, dims=2) ./ dt
    Plots.plot(title="Heteroclinic Acceleration", xlabel="X", ylabel="Y", zlabel="Z", seriestype=:scatter, markersize=2)
    Plots.plot!(accelerations[1, :], accelerations[2, :], accelerations[3, :], linewidth=1.2)
    Plots.savefig("heteroclinic_acceleration.png")
end

function plot_heteroclinic_force_field(path, dt, quiver_frequency=20)
    path = path[1:2, :]
    velocities = diff(path, dims=2) ./ dt
    accelerations = diff(velocities, dims=2) ./ dt
    ...
    forces_normalized = (forces)./maximum(forces) * 0.05
    print(length(forces_normalized))

    # Plot horizontal line
    x_values = 0:dt:(dt * (length(forces)))
    y_values = zeros(length(x_values)) .+ 0.05
    Plots.plot(x_values, zeros(length(x_values)), label="Horizontal Line", color="black", linewidth=1.2)

    # Plot quiver every quiver_frequency points
    quiver_indices = 1:quiver_frequency:length(x_values)
    print(forces_normalized[quiver_indices])
    print(x_values[quiver_indices])
    ...
    Plots.title!("Heteroclinic Force Field")
    Plots.xlabel!("Time")
    Plots.ylabel!("Force")
    Plots.savefig("heteroclinic_force_field.png")
end

function plot_heteroclinic_channel(source, sink, params, r=0.1, n=10, dt=0.001)
    ...
    initial_conditions = make_hypersphere(r, 3, n, [0.2,0.01,0.01])
    tspan = (0.0, 300.0)
    ...
    for u0 in initial_conditions
        ...
        filtered_sol = filter_solution(sol, x_index, y_index, 0.9, 0.999)
    ...
    line_prob = ODEProblem(vector_field!, initial_conditions[1], 300, params)
    line_sol = solve(line_prob, Tsit5(), reltol=1e-7, abstol=1e-7, saveat=dt)
    
    filtered_line_sol = filter_solution(line_sol, x_index, y_index, 0.999, 0.999)
    ...
    return filtered_line_sol
end

function plot_heteroclinic_channel_derivatives(source, sink, params, r=0.1, n=10, dt=0.001)
    max_dim_source = argmax(abs.(source))
    max_dim_sink = argmax(abs.(sink))
    ...
    initial_conditions = make_hypersphere(0.07, 3, n, [1,0,0])
    tspan = (0.0, 100.0)
    ...
    line_sol = solve(line_prob, Tsit5(), rtol=1e-6, atol=1e-6, saveat=dt)
...
sink = [0, 1, 0]
dt = 0.001
path = plot_heteroclinic_channel(source, sink, params, 0.01, 1, dt)
plot_heteroclinic_force_field(path, dt, 3000)
exit()
